...
export default class Aviso extends Component {
  ...
  render() {
    ...
    return (
      <Paper zIndex={5} style={{ margin: 10 }} onClick={() => !expand && this.expand(id, message, true)}>
        <Card key={id} zDepth={2} onExpandChange={value => this.expand(id, message, value)}>
          <CardHeader
          ...
          <Stepper activeStep={message.state} orientation="vertical">
            ...
            <Step active={expand && message.state >= 1}>
              ...
              <StepContent>
                ...
                <CardActions>
                  <RaisedButton primary disabled={loading} label="Enviar" onTouchTap={() => database.child('messages').child(messagesKey).update({ que, state: 2 })} />
                </CardActions>
              </StepContent>
            </Step>
